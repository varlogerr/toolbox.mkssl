...
# {SHLIB_GEN}
  ...
  tag_node_get() {
    ...
    [[ -n "${content}" ]] || return 1
    # there is no close tag if content last line is not close tag
    tail -n 1 -- <<< "${content}" | grep -q -- "${close_rex}" || return 1
...

{
  ...
  _opts_main() {
    ...
    declare -A overrides
    local overkeys_rex="$(print_decore "
      issuer-key
      issuer-cert
      days
      cn
...
help_description() {
  print_decore "Generate certificates"
}
...
CONF[hosts]="$(print_decore "${CONF[hosts]}")"
